...
void CMeshCube::MakeVex(void)
{
	...
	for (nCntHeight = 0; nCntHeight < 2; nCntHeight++, pVtx += 4)
	{
		...
		pVtx[0].tex = D3DXVECTOR2(0.25f*0.0f, (1.0f/3)*(2-nCntHeight));            // テクスチャ座標の設定
		pVtx[1].tex = D3DXVECTOR2(0.25f*1.0f, (1.0f/3)*(2-nCntHeight));            // テクスチャ座標の設定
		pVtx[2].tex = D3DXVECTOR2(0.25f*2.0f, (1.0f/3)*(2-nCntHeight));            // テクスチャ座標の設定
		pVtx[3].tex = D3DXVECTOR2(0.25f*3.0f, (1.0f/3)*(2-nCntHeight));            // テクスチャ座標の設定
	}
	// 鍵を開ける
	m_pVtxBuff->Unlock();
}
